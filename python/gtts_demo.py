...
def list_languages(voices):
    ...
    params:
        voices (dict(str,str)) - A dictionary of available languages and their corresponding codes
    returns:
        None
    """
...
def play_from_file(filename, language, tld):
    """
    Loads test messages from a supplied test file and runs them all through the TTS engine
    params:
...
def main():
    ...
    while command[0] != "q" and command[0] != "quit":
        if command[0] == "h" or command[0] == "help":
            ...
            if current_tld not in ACCENTS_SET:  # This does not mean it's a bad TLD, but just not one we can confirm works
